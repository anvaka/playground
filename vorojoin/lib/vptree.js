...
Copyright (c) 2013, François Pirsch
...
function nth_element(list, left, nth, right, comp) {
  if (nth <= 0 || nth > right - left + 1)
    throw (
      'VPTree.nth_element: nth must be in range [1, right-left+1] (nth=' +
      nth +
      ')'
    );
...
function selectVPIndex(list) {
  return Math.floor(random.nextDouble() * list.length);
}
...
module.exports.build = buildVPTree;
...
module.exports.load = function (S, distance, tree) {
  return new VPTree(S, distance, tree);
};
