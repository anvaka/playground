...
export default function createMapControls(drawContext) {
  ...
  let r = clamp(view.position[2], minR, maxR);
  ...
  const canvasRect = drawContext.canvas.getBoundingClientRect();
  ...
  function getZoomPlaneIntersection(clientX, clientY) {
    let viewPoint = view.getSceneCoordinate(clientX, clientY, canvasRect, drawContext.pixelRatio);
